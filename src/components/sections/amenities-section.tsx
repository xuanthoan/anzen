...
export function AmenitiesSection({ category = 'internal' }: { category?: 'internal' | 'external' | 'all' }) {
  ...
  return (
    <section id="tien-ich" className="bg-white py-20 md:py-28">
      <Container>
        <SectionHeading eyebrow="Tiện ích" title={title} description={category === 'external' ? 'Kết nối thuận tiện tới nhà trẻ, trạm sạc xe điện, shophouse và dịch vụ tiện ích liền kề, đáp ứng mọi nhu cầu sống của cư dân.' : 'Hệ thống tiện ích thể thao, không gian xanh và khu vực cộng đồng được thiết kế riêng cho cư dân, mang đến cuộc sống khỏe mạnh và kết nối.'} />
